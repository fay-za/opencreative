...
public final class GetStructureValueAction extends StructureAction {

    public GetStructureValueAction(Executor executor, Target target, int x, Arguments args) {
    ...
    protected void execute() {
        ...
        VariableLink targetVar = getArguments().getVariableLink("variable", this);
        ...
        for (Structure struct : getPlanet().getTerritory().getScript().getExecutors().getStructuresList()) {
            if (struct.getCallName().equalsIgnoreCase(structName)) {
                Object value = struct.getField(fieldName);
                setVarValue(targetVar, value);
                return;
            }
        }
    }
    ...
    public @NotNull ActionType getActionType() {
        return ActionType.STRUCT_GET_STRUCT_VALUE;
    }
}